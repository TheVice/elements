...
bool OGLES2HelloAPI_LinuxX11::Initialize()
{
	...
	if (!CreateNativeDisplay(&mNativeDisplay))
	{
		Cleanup();
		return false;
	}
	...
	if (!ChooseEGLConfig(mEglDisplay, mEglConfig))
	{
		Cleanup();
		return false;
	}
	...
	if (!CreateEGLSurface(mNativeWindow, mEglDisplay, mEglConfig, mEglSurface))
	{
		Cleanup();
		return false;
	}
	...
	if (!SetupEGLContext(mEglDisplay, mEglConfig, mEglSurface, mEglContext))
	{
		Cleanup();
		return false;
	}
