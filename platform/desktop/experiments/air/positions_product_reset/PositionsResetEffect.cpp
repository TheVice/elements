...
PositionsResetEffect::PositionsResetEffect(const std::vector<PositionsResetVertex>& vertices,
...
void PositionsResetEffect::construct(const std::vector<PositionsResetVertex>& vertices)
{
	assert(!vertices.empty());
...
void PositionsResetEffect::render(eps::rendering::program& program, short a_position, short index_count)
{
	EPS_STATE_VERTICES(texture_vertices_.get_product());
	EPS_STATE_INDICES(texture_indices_.get_product());
	//
	program.attribute_array(a_position, offsetof(PositionsResetVertex, a_vertex_xy), eps::math::vec2().length(),
							sizeof(PositionsResetVertex));
