#ifndef _POSITIONS_RESET_EFFECT_H_
#define _POSITIONS_RESET_EFFECT_H_
...
{
public:
	explicit PositionsResetEffect(const std::vector<PositionsResetVertex>& vertices,
								  const std::vector<GLubyte>& indices,
								  eps::rendering::buffer_usage usage = eps::rendering::buffer_usage::STATIC_DRAW);

	void construct(const std::vector<PositionsResetVertex>& vertices);
