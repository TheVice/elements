...
#include "PositionsResetControls.h"
#include <checkbox.h>
...
namespace Rendering
{
RTTI_DEFINITIONS(PositionsResetUi)

PositionsResetUi::PositionsResetUi(Library::Game& aGame, const std::string& aAssetPath) :
...
PositionsResetUi::~PositionsResetUi() = default;

bool PositionsResetUi::Initialize()
{
...
void PositionsResetUi::SetVertices(const std::vector<PositionsResetVertex>& aVertices)
{
	mVertices = aVertices;
	//
...
const std::vector<PositionsResetVertex>& PositionsResetUi::GetVertices() const
{
	return mVertices;
}

bool PositionsResetUi::IsNeedRestore() const
{
	return mIsRestoreNeed;
}

float& PositionsResetUi::GetValueBySliderId(int aSliderId)
{
	switch (aSliderId)
	{
