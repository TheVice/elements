...
PositionsProcessEffect::PositionsProcessEffect(const std::vector<PositionsProcessVertex>& vertices,
...
void PositionsProcessEffect::construct(const std::vector<PositionsProcessVertex>& vertices)
{
	assert(!vertices.empty());
...
void PositionsProcessEffect::render(eps::rendering::program& program, short a_position, short index_count)
{
	EPS_STATE_VERTICES(texture_vertices_.get_product());
	EPS_STATE_INDICES(texture_indices_.get_product());
	//
	program.attribute_array(a_position, offsetof(PositionsProcessVertex, a_vertex_xy), eps::math::vec2().length(),
							sizeof(PositionsProcessVertex));
