...
#include "PositionsProductResetControls.h"
#include <checkbox.h>
...
namespace Rendering
{
RTTI_DEFINITIONS(PositionsProductResetUi)

PositionsProductResetUi::PositionsProductResetUi(Library::Game& aGame, const std::string& aAssetPath) :
...
PositionsProductResetUi::~PositionsProductResetUi() = default;

bool PositionsProductResetUi::Initialize()
{
...
void PositionsProductResetUi::Update()
{
	UiAsset::Update();
	...
	if (auto directPanel = mControls[VERTEX_1_PANEL_9].lock())
	{
		if (directPanel->get_visible())
		{
			DISPLAY_FLOAT_VALUE_AT_LABEL(mVertices[1].a_vertex_xy.x, VERTEX_1_LABEL_6)
			DISPLAY_FLOAT_VALUE_AT_LABEL(mVertices[1].a_vertex_xy.y, VERTEX_1_LABEL_7)
		}
	}
	...
	if (auto directPanel = mControls[VERTEX_2_PANEL_14].lock())
	{
		if (directPanel->get_visible())
		{
			DISPLAY_FLOAT_VALUE_AT_LABEL(mVertices[2].a_vertex_xy.x, VERTEX_2_LABEL_11)
			DISPLAY_FLOAT_VALUE_AT_LABEL(mVertices[2].a_vertex_xy.y, VERTEX_2_LABEL_12)
		}
	}
	...
	if (auto directPanel = mControls[VERTEX_3_PANEL_19].lock())
	{
		if (directPanel->get_visible())
		{
			DISPLAY_FLOAT_VALUE_AT_LABEL(mVertices[3].a_vertex_xy.x, VERTEX_3_LABEL_16)
			DISPLAY_FLOAT_VALUE_AT_LABEL(mVertices[3].a_vertex_xy.y, VERTEX_3_LABEL_17)
		}
	}
}


void PositionsProductResetUi::SetVertices(const std::vector<PositionsProductResetVertex>& aVertices)
{
	mVertices = aVertices;
	//
	...
	SET_REAL_VALUE_AT_SLIDER(mVertices[1].a_vertex_xy.x, VERTEX_1_SLIDER_6)
	SET_REAL_VALUE_AT_SLIDER(mVertices[1].a_vertex_xy.y, VERTEX_1_SLIDER_7)
	SET_REAL_VALUE_AT_SLIDER(mVertices[2].a_vertex_xy.x, VERTEX_2_SLIDER_11)
	SET_REAL_VALUE_AT_SLIDER(mVertices[2].a_vertex_xy.y, VERTEX_2_SLIDER_12)
	SET_REAL_VALUE_AT_SLIDER(mVertices[3].a_vertex_xy.x, VERTEX_3_SLIDER_16)
	SET_REAL_VALUE_AT_SLIDER(mVertices[3].a_vertex_xy.y, VERTEX_3_SLIDER_17)
	//
	mIsRestoreNeed = false;
}

const std::vector<PositionsProductResetVertex>& PositionsProductResetUi::GetVertices() const
{
	return mVertices;
}

bool PositionsProductResetUi::IsNeedRestore() const
{
	return mIsRestoreNeed;
}

float& PositionsProductResetUi::GetValueBySliderId(int aSliderId)
{
	switch (aSliderId)
	{
