...
ParticlesProductProcessEffect::ParticlesProductProcessEffect(const std::vector<ParticlesProductProcessVertex>&
		vertices,
...
void ParticlesProductProcessEffect::construct(const std::vector<ParticlesProductProcessVertex>& vertices)
{
	assert(!vertices.empty());
...
void ParticlesProductProcessEffect::render(eps::rendering::program& program, short a_position,
		short index_count)
{
	EPS_STATE_VERTICES(texture_vertices_.get_product());
	EPS_STATE_INDICES(texture_indices_.get_product());
	//
	program.attribute_array(a_position, offsetof(ParticlesProductProcessVertex, a_vertex_xy),
							eps::math::vec2().length(), sizeof(ParticlesProductProcessVertex));
