...
FireEffect::FireEffect(const std::vector<FireVertex>& vertices,
...
void FireEffect::construct(const std::vector<FireVertex>& vertices)
{
	assert(!vertices.empty());
...
void FireEffect::render(eps::rendering::program& program, const std::array<short, 2>& a_position,
						short index_count)
{
	EPS_STATE_VERTICES(texture_vertices_.get_product());
	EPS_STATE_INDICES(texture_indices_.get_product());
	//
	program.attribute_array(std::get<0>(a_position), offsetof(FireVertex, a_vertex_xy),
							eps::math::vec2().length(), sizeof(FireVertex));
	program.attribute_array_enable(std::get<0>(a_position));
	program.attribute_array(std::get<1>(a_position), offsetof(FireVertex, a_vertex_uv),
							eps::math::vec2().length(), sizeof(FireVertex));
