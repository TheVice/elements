#ifndef _TONE_EFFECT_H_
#define _TONE_EFFECT_H_
...
{
public:
	explicit ToneEffect(const std::vector<ToneVertex>& vertices,
						const std::vector<GLubyte>& indices,
						eps::rendering::buffer_usage usage = eps::rendering::buffer_usage::STATIC_DRAW);

	void construct(const std::vector<ToneVertex>& vertices);
