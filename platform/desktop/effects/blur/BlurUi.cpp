...
namespace Rendering
{
RTTI_DEFINITIONS(BlurUi)

BlurUi::BlurUi(Library::Game& aGame, const std::string& aAssetPath) :
...
BlurUi::~BlurUi() = default;

bool BlurUi::Initialize()
{
	...
	IS_CONTROL_EXIST(U_OFFSET_BUTTON_3)
	...
	//
	IS_CONTROL_EXIST(U_OFFSET_PANEL_4)
	...
	//
	IS_CONTROL_EXIST(U_OFFSET_LABEL_1)
	IS_CONTROL_EXIST(U_OFFSET_LABEL_2)
	IS_CONTROL_EXIST(U_OFFSET_LABEL_5)
	...
	if (auto directButton = std::static_pointer_cast<eps::ui::button>(mControls[U_OFFSET_BUTTON_3].lock()))
	{
		directButton->set_click([this](eps::ui::state)
		{
			if (auto directPanel = mControls[U_OFFSET_PANEL_4].lock())
			{
				SET_VISIBLE(!directPanel->get_visible(), U_OFFSET_PANEL_4)
			}
		});
	}
...
void BlurUi::Update()
{
	...
	if (auto directPanel = mControls[U_OFFSET_PANEL_4].lock())
	{
		...
		{
			DISPLAY_FLOAT_VALUE_AT_LABEL(m_u_offset[0], U_OFFSET_LABEL_1)
			DISPLAY_FLOAT_VALUE_AT_LABEL(m_u_offset[1], U_OFFSET_LABEL_2)
		}
	}
...
void BlurUi::Set_u_offset(const eps::math::vec2& a_u_offset)
{
	m_u_offset = a_u_offset;
	//
	SET_REAL_VALUE_AT_SLIDER(m_u_offset[0], U_OFFSET_SLIDER_1)
	SET_REAL_VALUE_AT_SLIDER(m_u_offset[1], U_OFFSET_SLIDER_2)
	//
	mIsRestoreNeed = false;
}

const eps::math::vec2& BlurUi::Get_u_offset() const
{
	return m_u_offset;
}

void BlurUi::SetVertices(const std::vector<BlurVertex>& aVertices)
{
	mVertices = aVertices;
	//
...
const std::vector<BlurVertex>& BlurUi::GetVertices() const
{
	return mVertices;
}

bool BlurUi::IsNeedRestore() const
{
	return mIsRestoreNeed;
}

float& BlurUi::GetValueBySliderId(int aSliderId)
{
	switch (aSliderId)
	{
		case U_OFFSET_SLIDER_1:
			return m_u_offset[0];

		case U_OFFSET_SLIDER_2:
			return m_u_offset[1];
