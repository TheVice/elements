...
#define CLEAR()	\
	...
	\
	u_source.clear();	\
	u_offset = eps::math::vec2();	\
	\
	mIsEmpty = true;

bool BlurSettings::read(const pugi::xml_document& doc)
{
	CLEAR()
	...
	const auto u_offset_node = settings_node.child("u_offset");

	if (u_offset_node.empty())
	{
		return !mIsEmpty;
	}
	...
	for (const auto& subNode : vertices_node)
	{
		...
		const auto vertex = BlurVertex(a_vertex_xy, a_vertex_uv);
	...
	if (!Library::ReaderHelpers::read_glm_vec(u_offset_node, u_offset))
	{
		return !mIsEmpty;
	}
