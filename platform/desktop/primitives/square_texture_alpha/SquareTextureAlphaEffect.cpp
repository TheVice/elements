...
SquareTextureAlphaEffect::SquareTextureAlphaEffect(const std::vector<SquareTextureAlphaVertex>& vertices,
...
void SquareTextureAlphaEffect::construct(const std::vector<SquareTextureAlphaVertex>& vertices)
{
	assert(!vertices.empty());
...
void SquareTextureAlphaEffect::render(eps::rendering::program& program,
									  const std::array<short, 2>& a_position, short index_count)
{
	EPS_STATE_VERTICES(texture_vertices_.get_product());
	EPS_STATE_INDICES(texture_indices_.get_product());
	//
	program.attribute_array(std::get<0>(a_position), offsetof(SquareTextureAlphaVertex, a_vertex_xy),
							eps::math::vec2().length(), sizeof(SquareTextureAlphaVertex));
	program.attribute_array_enable(std::get<0>(a_position));
	program.attribute_array(std::get<1>(a_position), offsetof(SquareTextureAlphaVertex, a_vertex_uv),
							eps::math::vec2().length(), sizeof(SquareTextureAlphaVertex));
