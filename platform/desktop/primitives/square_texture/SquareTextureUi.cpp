...
#include "SquareTextureControls.h"
#include <checkbox.h>
...
namespace Rendering
{
RTTI_DEFINITIONS(SquareTextureUi)

SquareTextureUi::SquareTextureUi(Library::Game& aGame, const std::string& aAssetPath) :
...
SquareTextureUi::~SquareTextureUi() = default;

bool SquareTextureUi::Initialize()
{
	...
	IS_CONTROL_EXIST(VERTEX_0_BUTTON_20)
	IS_CONTROL_EXIST(VERTEX_1_BUTTON_27)
	IS_CONTROL_EXIST(VERTEX_2_BUTTON_34)
	IS_CONTROL_EXIST(VERTEX_3_BUTTON_41)
	IS_CONTROL_EXIST(RESTORE_BUTTON)
	//
	//
	IS_CONTROL_EXIST(U_TRANSFORM_PANEL_14)
	IS_CONTROL_EXIST(VERTEX_0_PANEL_21)
	IS_CONTROL_EXIST(VERTEX_1_PANEL_28)
	IS_CONTROL_EXIST(VERTEX_2_PANEL_35)
	IS_CONTROL_EXIST(VERTEX_3_PANEL_42)
	//
	...
	if (auto directButton = std::static_pointer_cast<eps::ui::button>(mControls[VERTEX_0_BUTTON_20].lock()))
	{
		directButton->set_click([this](eps::ui::state)
		{
			if (auto directPanel = mControls[VERTEX_0_PANEL_21].lock())
			{
				SET_VISIBLE(!directPanel->get_visible(), VERTEX_0_PANEL_21)
				SET_VISIBLE(false, VERTEX_1_PANEL_28)
				SET_VISIBLE(false, VERTEX_2_PANEL_35)
				SET_VISIBLE(false, VERTEX_3_PANEL_42)
			}
		});
	}
	...
	if (auto directButton = std::static_pointer_cast<eps::ui::button>(mControls[VERTEX_1_BUTTON_27].lock()))
	{
		directButton->set_click([this](eps::ui::state)
		{
			if (auto directPanel = mControls[VERTEX_1_PANEL_28].lock())
			{
				SET_VISIBLE(false, VERTEX_0_PANEL_21)
				SET_VISIBLE(!directPanel->get_visible(), VERTEX_1_PANEL_28)
				SET_VISIBLE(false, VERTEX_2_PANEL_35)
				SET_VISIBLE(false, VERTEX_3_PANEL_42)
			}
		});
	...
	if (auto directButton = std::static_pointer_cast<eps::ui::button>(mControls[VERTEX_2_BUTTON_34].lock()))
	{
		directButton->set_click([this](eps::ui::state)
		{
			if (auto directPanel = mControls[VERTEX_2_PANEL_35].lock())
			{
				SET_VISIBLE(false, VERTEX_0_PANEL_21)
				SET_VISIBLE(false, VERTEX_1_PANEL_28)
				SET_VISIBLE(!directPanel->get_visible(), VERTEX_2_PANEL_35)
				SET_VISIBLE(false, VERTEX_3_PANEL_42)
			}
		});
	}

	if (auto directButton = std::static_pointer_cast<eps::ui::button>(mControls[VERTEX_3_BUTTON_41].lock()))
	{
		directButton->set_click([this](eps::ui::state)
		{
			if (auto directPanel = mControls[VERTEX_3_PANEL_42].lock())
			{
				SET_VISIBLE(false, VERTEX_0_PANEL_21)
				SET_VISIBLE(false, VERTEX_1_PANEL_28)
				SET_VISIBLE(false, VERTEX_2_PANEL_35)
				SET_VISIBLE(!directPanel->get_visible(), VERTEX_3_PANEL_42)
			}
		});
	}
...
void SquareTextureUi::Update()
{
	...
	if (auto directPanel = mControls[VERTEX_0_PANEL_21].lock())
	{
		if (directPanel->get_visible())
		{
			DISPLAY_FLOAT_VALUE_AT_LABEL(mVertices[0].a_vertex_xy.x, VERTEX_0_LABEL_16)
			DISPLAY_FLOAT_VALUE_AT_LABEL(mVertices[0].a_vertex_xy.y, VERTEX_0_LABEL_17)
			DISPLAY_FLOAT_VALUE_AT_LABEL(mVertices[0].a_vertex_uv.x, VERTEX_0_LABEL_18)
			DISPLAY_FLOAT_VALUE_AT_LABEL(mVertices[0].a_vertex_uv.y, VERTEX_0_LABEL_19)
		}
	}
	...
	if (auto directPanel = mControls[VERTEX_1_PANEL_28].lock())
	{
		if (directPanel->get_visible())
		{
			DISPLAY_FLOAT_VALUE_AT_LABEL(mVertices[1].a_vertex_xy.x, VERTEX_1_LABEL_23)
			DISPLAY_FLOAT_VALUE_AT_LABEL(mVertices[1].a_vertex_xy.y, VERTEX_1_LABEL_24)
			DISPLAY_FLOAT_VALUE_AT_LABEL(mVertices[1].a_vertex_uv.x, VERTEX_1_LABEL_25)
			DISPLAY_FLOAT_VALUE_AT_LABEL(mVertices[1].a_vertex_uv.y, VERTEX_1_LABEL_26)
		}
	}
	...
	if (auto directPanel = mControls[VERTEX_2_PANEL_35].lock())
	{
		if (directPanel->get_visible())
		{
			...
			DISPLAY_FLOAT_VALUE_AT_LABEL(mVertices[2].a_vertex_uv.x, VERTEX_2_LABEL_32)
			DISPLAY_FLOAT_VALUE_AT_LABEL(mVertices[2].a_vertex_uv.y, VERTEX_2_LABEL_33)
		}
	}
	...
	if (auto directPanel = mControls[VERTEX_3_PANEL_42].lock())
	{
		if (directPanel->get_visible())
		{
			DISPLAY_FLOAT_VALUE_AT_LABEL(mVertices[3].a_vertex_xy.x, VERTEX_3_LABEL_37)
			DISPLAY_FLOAT_VALUE_AT_LABEL(mVertices[3].a_vertex_xy.y, VERTEX_3_LABEL_38)
			DISPLAY_FLOAT_VALUE_AT_LABEL(mVertices[3].a_vertex_uv.x, VERTEX_3_LABEL_39)
			DISPLAY_FLOAT_VALUE_AT_LABEL(mVertices[3].a_vertex_uv.y, VERTEX_3_LABEL_40)
		}
	}
}

void SquareTextureUi::Set_u_transform(const eps::math::mat4& a_u_transform)
{
...
const eps::math::mat4& SquareTextureUi::Get_u_transform() const
{
	return m_u_transform;
}

void SquareTextureUi::SetVertices(const std::vector<SquareTextureVertex>& aVertices)
{
	mVertices = aVertices;
	//
	SET_REAL_VALUE_AT_SLIDER(mVertices[0].a_vertex_xy.x, VERTEX_0_SLIDER_16)
	SET_REAL_VALUE_AT_SLIDER(mVertices[0].a_vertex_xy.y, VERTEX_0_SLIDER_17)
	SET_REAL_VALUE_AT_SLIDER(mVertices[0].a_vertex_uv.x, VERTEX_0_SLIDER_18)
	SET_REAL_VALUE_AT_SLIDER(mVertices[0].a_vertex_uv.y, VERTEX_0_SLIDER_19)
	SET_REAL_VALUE_AT_SLIDER(mVertices[1].a_vertex_xy.x, VERTEX_1_SLIDER_23)
	SET_REAL_VALUE_AT_SLIDER(mVertices[1].a_vertex_xy.y, VERTEX_1_SLIDER_24)
	SET_REAL_VALUE_AT_SLIDER(mVertices[1].a_vertex_uv.x, VERTEX_1_SLIDER_25)
	SET_REAL_VALUE_AT_SLIDER(mVertices[1].a_vertex_uv.y, VERTEX_1_SLIDER_26)
	...
	SET_REAL_VALUE_AT_SLIDER(mVertices[2].a_vertex_uv.x, VERTEX_2_SLIDER_32)
	SET_REAL_VALUE_AT_SLIDER(mVertices[2].a_vertex_uv.y, VERTEX_2_SLIDER_33)
	SET_REAL_VALUE_AT_SLIDER(mVertices[3].a_vertex_xy.x, VERTEX_3_SLIDER_37)
	SET_REAL_VALUE_AT_SLIDER(mVertices[3].a_vertex_xy.y, VERTEX_3_SLIDER_38)
	SET_REAL_VALUE_AT_SLIDER(mVertices[3].a_vertex_uv.x, VERTEX_3_SLIDER_39)
	SET_REAL_VALUE_AT_SLIDER(mVertices[3].a_vertex_uv.y, VERTEX_3_SLIDER_40)
	//
	mIsRestoreNeed = false;
}

const std::vector<SquareTextureVertex>& SquareTextureUi::GetVertices() const
{
	return mVertices;
}

bool SquareTextureUi::IsNeedRestore() const
{
	return mIsRestoreNeed;
}

float& SquareTextureUi::GetValueBySliderId(int aSliderId)
{
	switch (aSliderId)
	{
